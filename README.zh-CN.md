...
- 可配置刷新间隔
- 按 `g` 手动刷新
- 按 `C-c C-k` 查看最近 15 个交易日 K 线图
- 在 K 线日期上按 `C-c C-m` 查看当天分时图
- 绘制 5/10/15/20/30/60 日均线图
- 按 `q` 停止刷新并退出
- 高亮涨跌数值
- 支持涨跌幅阈值提醒
...
K 线会自动按最长均线拉取足够历史数据。默认配置下会请求 119 条日线，
这样 MA60 也能显示 60 个样本点。
...
从本地源码开发时，可以用下面的命令一次性重载所有拆分后的模块：
